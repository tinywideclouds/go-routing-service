...
// Enqueue saves a single message envelope to cold storage using the provided messageID.
// It uses the definitive messageID generated by the Router as the Document ID to ensure idempotency.
func (s *FirestoreColdQueue) Enqueue(ctx context.Context, messageID string, envelope *secure.SecureEnvelope) error {
	...
	// We use Set (overwrite) instead of Create. If the message exists (e.g. retry), we update it.
	// Since the ID and content are immutable for a specific message instance, this is safe.
